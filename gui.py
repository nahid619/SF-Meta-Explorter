...
from config import DEFAULT_PICKLIST_FILENAME, DEFAULT_METADATA_FILENAME
from salesforce_client import SalesforceClient
from picklist_exporter import PicklistExporter
from metadata_exporter import MetadataExporter
from utils import format_runtime, print_picklist_statistics, print_metadata_statistics
...
class SalesforceExporterGUI(ctk.CTk):
    ...
    def login_action(self):
        """Handle login button click - FIXED: Now runs in background thread"""
        # Validate inputs first on main thread
        ...
        # FIXED: Run connection in background thread
        def login_thread():
            try:
                ...
                sf_client = SalesforceClient(
                    ...
                    domain=domain,
                    status_callback=self.thread_safe_status_update
                )
                
                # Initialize exporters
                picklist_exporter = PicklistExporter(sf_client)
                metadata_exporter = MetadataExporter(sf_client)
                
                # Get all objects (this can take time)
                all_org_objects = sf_client.get_all_objects()
                
                # FIXED: Schedule UI update on main thread
                self.after(0, lambda: self._on_login_success(
                    sf_client, picklist_exporter, metadata_exporter, all_org_objects
                ))
                
            except Exception as e:
                # FIXED: Schedule error handling on main thread
                self.after(0, lambda: self._on_login_error(str(e)))
        
        # Start the background thread
        self.active_thread = threading.Thread(target=login_thread, daemon=True)
        self.active_thread.start()
    
    def _on_login_success(self, sf_client, picklist_exporter, metadata_exporter, all_org_objects):
        """Called on main thread after successful login"""
        self.sf_client = sf_client
        self.picklist_exporter = picklist_exporter
        self.metadata_exporter = metadata_exporter
        self.all_org_objects = all_org_objects
        ...
        # Re-enable login button in case user logs out
    ...
    def _setup_export_frame(self):
        ...
        self.export_metadata_button.grid(row=0, column=1, sticky="ew", padx=(5, 0))
    ...
    # ==================================
    # Export Action Methods - FIXED WITH THREADING
    # ==================================
    
    def export_picklist_action(self):
        """Handle export picklist button click - FIXED: Now runs in background thread"""
        ...
        # FIXED: Check if another operation is running
        ...
        def export_thread():
            ...
            try:
                ...
                # FIXED: Schedule error handling on main thread
                self.after(0, lambda: self._on_export_error(str(e), "picklist"))
            finally:
                # Release the lock
                self.operation_lock.release()
        
        # Start the background thread
        self.active_thread = threading.Thread(target=export_thread, daemon=True)
        self.active_thread.start()
    
    def _on_picklist_export_success(self, output_path, stats, runtime_formatted):
        """Called on main thread after successful picklist export"""
        self.thread_safe_status_update(f"Export Complete! Total Runtime: {runtime_formatted}")
        messagebox.showinfo(
            "Export Done", 
    ...
    def export_metadata_action(self):
        """Handle export metadata button click - FIXED: Now runs in background thread"""
        ...
        def export_thread():
            ...
            try:
                ...
                # FIXED: Schedule error handling on main thread
                self.after(0, lambda: self._on_export_error(str(e), "metadata"))
            finally:
                # Release the lock
                self.operation_lock.release()
    ...
    def _on_metadata_export_success(self, output_path, stats, runtime_formatted):
        """Called on main thread after successful metadata export"""
        self.thread_safe_status_update(f"Export Complete! Total Runtime: {runtime_formatted}")
        messagebox.showinfo(
            "Export Done", 
    ...
    def _on_export_error(self, error_message, export_type):
        """Called on main thread after export failure"""
        self.thread_safe_status_update(f"❌ FATAL EXPORT ERROR: {error_message}")
        messagebox.showerror("Export Error", f"A fatal error occurred during export: {error_message}")
        
        # Re-enable buttons based on export type
        if export_type == "picklist":
            self.export_picklist_button.configure(state="normal", text="Export Picklist Data")
        else:
            self.export_metadata_button.configure(state="normal", text="Export Metadata")
    ...
    # ==================================
    # Utility Methods - FIXED FOR THREAD SAFETY
    # ==================================
    ...
    def thread_safe_status_update(self, message: str, verbose: bool = False):
        """
        ADDED: Thread-safe status update method
        Can be called from background threads
        """
        # Schedule the update on the main thread
        self.after(0, lambda: self.update_status(message, verbose))
    
    def logout_action(self):
        """Clears connection, resets state, and returns to the login screen"""
        # FIXED: Check if an operation is in progress
